document.addEventListener('DOMContentLoaded', function () {
    const loadButtonWrapper = document.querySelector('.truthnews-advanced-pagination');

    if (!loadButtonWrapper) return;

    const loadButton = loadButtonWrapper.querySelector('.truthnews-ajax-load-button');
    const loader = loadButtonWrapper.querySelector('.truthnews-pagination-spinner');
    ...
    const fetchThePosts = () => {
        ...
        const data = {
            action: 'truthnews_load_posts',
            nonce: TruthnewsVars.load_post_nonce_wp,
            query_vars: TruthnewsVars.query_vars,
            page: currentPage + 1,
            post_type: 'post' // Adjust this if you're loading a different post type
        };

        fetch(TruthnewsVars.ajaxurl, {
            method: 'POST',
